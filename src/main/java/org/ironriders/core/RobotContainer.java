...
public class RobotContainer {
	...
	private void configureBindings() {
		...
		secondaryController.button(ControllerConstants.scoreAlgae).onTrue(robotCommands.scoreAlgae());
		secondaryController.button(ControllerConstants.scoreCoral).onTrue(robotCommands.scoreCoral(GameState.getCoralTarget()));

		secondaryController.axisGreaterThan(ControllerConstants.grabAlgaeTrigger, 0.5).onTrue(Commands.runOnce(() -> {
